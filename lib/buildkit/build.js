...
parser.addOption("s", "stepbystep", "", "compress files step by step");
...
exports.main = function main(args) {
    ...
    for (var section in sections) {
        ...
        if (options.list) {
            ...
            if (('stepbystep' in options)
                && !('uncompressed' in options)) {
                concat = MERGE.concat(group, true);
            }else{
                concat = MERGE.concat(group);
            }
            if (!('uncompressed' in options)
                    || ('stepbystep' in options)) {
